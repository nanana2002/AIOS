...
mcp = FastMCP(name="mofa-tools-server")
...
@mcp.tool(
    ...
    description=(
            ...
            "向 LLM 传入用户输入脚本、需求或描述，由 LLM 模型生成符合 MoFA DataFlow 或 Node 定义规范的内容。"
    )
)
async def generate_mofa_node(
    ...
    """
    client = OpenAI(
        base_url=os.getenv("OPENAI_BASE_URL", "http://127.0.0.1:8005/v1"),
